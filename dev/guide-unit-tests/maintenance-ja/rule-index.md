...
このファイルは Rule ID と、そのルールを含む実行時資料を対応付けるための索引である。
...
優先度: Essential。
参照元グループ: `khorikov-core`。
判断内容: 共有依存と揮発性依存は、単体テストでは制御できる形に置き換える。プロセス内で安定して動く協力オブジェクトは、実用上問題がなければ実物のまま使う。
...
優先度: Recommended。
参照元グループ: `khorikov-core`。
判断内容: テストは直線的に保ち、アサーションにプロダクションコードのようなロジックを入れない。
...
優先度: Essential。
参照元グループ: `khorikov-core`。
判断内容: テストのためだけにプロダクションコードを汚染せず、本番モデルも改善する設計変更を優先する。
