...
10. 選択した変更に複数の独立した意味単位が含まれている場合は、論理的なコミット単位に分け、それぞれのコミットについて候補と推薦を提示する。コミットを分割したほうが明確な場合は、そのことを明示する。
...
ユーザーが別の指定をしない限り、回答本文はユーザーの言語に合わせる。ただし、コミットメッセージ自体は英語のままにする。通常は、簡潔な箇条書きが最適だ。
...
ステージング済み変更がない場合は、フォールバックしたことを明示する。
