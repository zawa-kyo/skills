...
This repository publishes reusable agent skills with `apm`. Each directory under `dev/`, `thinking/`, or `writing/` is an independent skill package.
...
- each skill's paired `SKILL.md` and `SKILL-ja.md` files under `dev/<skill>/`, `thinking/<skill>/`, or `writing/<skill>/`
...
- the relevant `dev/<skill>/`, `thinking/<skill>/`, or `writing/<skill>/` directory contents
...
- Write Japanese skill definitions in plain form by default, using `である` / `する` style instead of polite `です` / `ます` style. Keep the prose readable rather than stiff. Preserve polite wording only inside examples where the user-facing output itself should be polite.
